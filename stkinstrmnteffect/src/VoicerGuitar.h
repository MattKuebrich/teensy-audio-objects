#ifndef STK_VoicerGuitar_H
#define STK_VoicerGuitar_H
...
class VoicerGuitar : public Stk
{
 public:
  //! Class constructor taking an optional note decay time (in seconds).
  VoicerGuitar( StkFloat decayTime = 0.2 ); // 0.2
  ...
  */
  void addInstrument( Guitar *instrument, int group=0 );
  ...
  */
  void removeInstrument( Guitar *instrument );
...
inline StkFloat VoicerGuitar :: lastOut( unsigned int channel )
{
#if defined(_STK_DEBUG_)
  if ( channel >= lastFrame_.channels() ) {
    oStream_ << "VoicerGuitar::lastOut(): channel argument is invalid!";
...
inline StkFloat VoicerGuitar :: tick( unsigned int channel )
{
  unsigned int j;
  ...
  //return lastFrame_[channel] / getNumActiveVoices();
  return lastFrame_[channel];
}

inline StkFrames& VoicerGuitar :: tick( StkFrames& frames, unsigned int channel )
{
  unsigned int nChannels = lastFrame_.channels();
#if defined(_STK_DEBUG_)
  if ( channel > frames.channels() - nChannels ) {
    oStream_ << "VoicerGuitar::tick(): channel and StkFrames arguments are incompatible!";
    handleError( StkError::FUNCTION_ARGUMENT );
  }
#endif
